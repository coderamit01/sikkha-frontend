...
const AvailabilityButton = ({ available }: { available: IAvailability }) => {
  ...
  return (
    <button
      ...
      className={`h-10 px-2 py-2 rounded-xl text-sm font-semibold transition-all duration-200 text-left flex items-center gap-2 bg-[#25a5a21c] text-brand cursor-pointer ${isSelected ? "bg-brand text-white" : ""} ${isBooked ? "bg-slate-200 text-slate-800" : "hover:bg-brand hover:text-white"}`}
    >
      <Clock size={16} />
      <span>
        {start} · {end}
